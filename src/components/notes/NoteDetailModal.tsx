...
export const NoteDetailModal: React.FC<NoteDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-black/80 sm:backdrop-blur-md overflow-hidden">
      <div className="relative w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-3xl bg-[#0c0e15] sm:border sm:border-white/[0.12] sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col font-sans">
        {/* TOP BAR */}
        <div className="px-4 sm:px-6 py-3.5 border-b border-white/[0.08] flex items-center justify-between bg-white/[0.02] shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 flex items-center justify-center shrink-0">
              {note.type === 'sermon' && <Church className="w-4 h-4 sm:w-5 sm:h-5" />}
              {note.type === 'book' && <BookOpen className="w-4 h-4 sm:w-5 sm:h-5" />}
              {note.type === 'bible_study' && <Cross className="w-4 h-4 sm:w-5 sm:h-5" />}
              {note.type === 'general' && <Layers className="w-4 h-4 sm:w-5 sm:h-5" />}
            </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/30 text-xs font-semibold transition cursor-pointer"
            ...
            <button
              ...
              className="p-1.5 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 hover:text-rose-300 transition cursor-pointer"
              title="Delete Note"
            >
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full bg-white/[0.06] hover:bg-white/[0.12] flex items-center justify-center text-slate-300 hover:text-white transition cursor-pointer ml-1"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-200">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-400">
            ...
            {/* Book hierarchy badges */}
            {note.type === 'book' && note.bookTitle && (
              <div className="px-2 py-0.5 rounded-md bg-teal-500/10 text-teal-300 border border-teal-500/20 font-medium">
                Book: {note.bookTitle}
              </div>
            )}
            {note.type === 'book' && note.chapter && (
              <div className="px-2 py-0.5 rounded-md bg-white/[0.06] text-slate-300 border border-white/10">
                {note.chapter}
              </div>
            )}
            {note.type === 'book' && note.pageRange && (
              <div className="px-2 py-0.5 rounded-md bg-white/[0.06] text-slate-400 border border-white/10">
                {note.pageRange}
              </div>
            )}

            {/* Sermon series badge */}
            {note.type === 'sermon' && note.seriesName && (
              <div className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-medium">
                Series: {note.seriesName}
              </div>
            )}
            {note.type === 'sermon' && !note.seriesName && (
              <div className="px-2 py-0.5 rounded-md bg-white/[0.06] text-slate-400 border border-white/10 text-[10px]">
                Standalone Sermon
              </div>
            )}

            {/* Bible study dynamic book badge */}
            {note.type === 'bible_study' && note.bibleBook && (
              <div className="px-2 py-0.5 rounded-md bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 font-medium">
                Bible Book: {note.bibleBook}
              </div>
            )}
            {note.type === 'bible_study' && note.bibleChapter && (
              <div className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 font-medium">
                {note.bibleChapter}
              </div>
            )}

            {/* Normal Note Category badge */}
            {(note.type === 'normal' || note.type === 'general') && (note.categoryName || note.tags?.[0]) && (
              <div className="px-2 py-0.5 rounded-md bg-indigo-500/15 text-indigo-300 border border-indigo-500/30 font-medium">
                {note.categoryName || note.tags?.[0]}
              </div>
            )}

            {note.sourceTitle && !note.bookTitle && !note.seriesName && !note.bibleBook && (
