...
import { syncGlobalUser } from './appsService';

export const APP_ID = 'budget';

/**
 * In the Bakayise Assistant ecosystem, budget sub-collections live under /apps/budget/{collectionName}.
 * Root collections /apps and /users are preserved at the root.
 */
function collection(
  firestore: typeof db,
  path: string,
  ...
  if (path === 'apps' || path === 'users') {
    return firestoreCollection(firestore, path, ...pathSegments);
  }
  return firestoreCollection(firestore, 'apps', APP_ID, path, ...pathSegments);
}
...
function doc(
  ...
  return firestoreDoc(firestore, 'apps', APP_ID, path, ...pathSegments);
}

export function getAppColRef(colName: string, appId: string = APP_ID) {
  return firestoreCollection(db, 'apps', appId, colName);
}

export function getAppDocRef(colName: string, docId: string, appId: string = APP_ID) {
  return firestoreDoc(db, 'apps', appId, colName, docId);
}
...
/**
 * Safely handles Firestore snapshot errors, dispatching to onError callbacks while
 * suppressing noisy console.warn toasts for permission restrictions or offline states.
 */
export function handleSnapshotError(
  ...
  if (!isPermissionDenied) {
    console.warn(`Firestore onSnapshot error [${colTag}]:`, error?.message || error);
  }
}
...
export function subscribeToAccounts(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('financial_accounts', error, onError);
    }
  );
}
...
export function subscribeToBudgetPeriods(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('budget_periods', error, onError);
    }
  );
}
...
export function subscribeToIncomes(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('incomes', error, errorCallback);
    }
  );
}
...
export function subscribeToAllWorkspaceIncomes(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('all_incomes', error, onError);
    }
  );
}
...
export function subscribeToCategories(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('budget_categories', error, errorCallback);
    }
  );
}
...
export function subscribeToAllWorkspaceCategories(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('all_categories', error, onError);
    }
  );
}
...
export function subscribeToExpenses(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('expenses', error, errorCallback);
    }
  );
}
...
export function subscribeToAllWorkspaceExpenses(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('all_expenses', error, onError);
    }
  );
}
...
export function subscribeToDebts(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('debts', error, onError);
    }
  );
}
...
export function subscribeToBabyStepsState(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError(`baby_steps_state/${householdId}`, error, onError);
    }
  );
}
...
export function subscribeToEmergencyFundLogs(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('emergency_fund_logs', error, onError);
    }
  );
}
...
export async function syncUserProfile(): Promise<void> {
  try {
    ...
    // Also sync global user profile in root /users/{uid} for cross-app access
    await syncGlobalUser(currentUser, member);
  } catch (error: any) {
    const isPerm =
      error?.code === 'permission-denied' ||
      (typeof error?.message === 'string' &&
...
export function subscribeToArchivedWorksheets(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('archived_worksheets', error, onError);
    }
  );
}
...
export function subscribeToAllWorkspaces(
  ...
  return onSnapshot(
    ...
    (error) => {
      handleSnapshotError('workspaces', error, onError);
    }
  );
}
