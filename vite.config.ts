...
9. "notes": Summarize itemized contents or context.`;
      ...
      server.middlewares.use('/api/scan-income', async (req, res) => {
        if (req.method !== 'POST') {
          res.statusCode = 405;
          return res.end('Method not allowed');
        }
        ...
        req.on('end', async () => {
          try {
            ...
            if (!apiKey) {
              res.statusCode = 400;
              res.setHeader('Content-Type', 'application/json');
              return res.end(JSON.stringify({ error: 'GEMINI_API_KEY environment variable is missing on server.' }));
            }
            ...
            if (!text) {
              res.statusCode = 400;
              res.setHeader('Content-Type', 'application/json');
              return res.end(JSON.stringify({ error: 'Text payload is required.' }));
            }
...
7. "notes": Brief note describing the parsed entry.`;
            ...
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            return res.end(JSON.stringify({ isBulk, incomes: formattedIncomes }));
          } catch (err: any) {
            console.error('Error in /api/scan-income:', err);
            res.statusCode = 500;
            res.setHeader('Content-Type', 'application/json');
            return res.end(JSON.stringify({ error: err?.message || 'Failed to parse income entries' }));
          }
        });
      });

      server.middlewares.use('/api/scan-categories', async (req, res) => {
        if (req.method !== 'POST') {
          res.statusCode = 405;
          return res.end('Method not allowed');
        }
...
7. "notes": Brief note or details.`;
            ...
            res.statusCode = 200;
            res.setHeader('Content-Type', 'application/json');
            return res.end(JSON.stringify({ isBulk, categories: formattedCategories }));
          } catch (err: any) {
            console.error('Error in /api/scan-categories:', err);
            res.statusCode = 500;
            res.setHeader('Content-Type', 'application/json');
            return res.end(JSON.stringify({ error: err?.message || 'Failed to parse budget categories' }));
          }
        });
      });

      server.middlewares.use('/api/meals/suggest-ai', async (req, res) => {
        if (req.method !== 'POST') {
          res.statusCode = 405;
          return res.end('Method not allowed');
        }
...
2. Ensure realistic South African & global family homestyle flavors.
3. If "pantry_challenge" mode is requested, maximize the use of ingredients in pantry and indicate pantryMatchPercentage.
4. Include accurate estimated cost in South African Rands (ZAR).
5. Output STRICTLY a JSON object.`;
                ...
                if (jsonText) {
                  ...
                  res.statusCode = 200;
                  res.setHeader('Content-Type', 'application/json');
                  return res.end(JSON.stringify({ ...parsed, modelUsed: 'gemini-3.1-flash-lite' }));
                }
              } catch (geminiErr: any) {
                console.warn('Gemini meal suggestion call failed, falling back:', geminiErr?.message || geminiErr);
              }
            }
            ...
            res.statusCode = 500;
            res.setHeader('Content-Type', 'application/json');
            return res.end(JSON.stringify({ error: err?.message || 'Failed to generate meal suggestions' }));
          }
        });
      });
